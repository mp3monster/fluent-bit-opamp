...
usage() {
  cat <<'EOF'
Usage: ./mcp/configure-codex-fastmcp.sh [options]

Configure Codex CLI to run the OpAMP MCP server via fastmcp.

Options:
  --name <value>          Codex MCP server name (default: opamp-server)
...
if codex mcp get "${SERVER_NAME}" >/dev/null 2>&1; then
  echo "Existing Codex MCP server '${SERVER_NAME}' found. Replacing..."
...
echo "Codex MCP server '${SERVER_NAME}' has been configured."
